...
namespace PizzaApplication.UI
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            // load DB or deserialize
            var tryLoadDatabase = false;
            var tryDeserialize = false;
            ...
            Storefront RestonStorefront = new Storefront("Reston");
            Storefront HerndonStorefront = new Storefront("Herndon");
            Storefront SterlingStorefront = new Storefront("Sterling");
            Storefront currentStorefront;

            // initialize store inventories
            List<Inventory> inventoryList = new List<Inventory>();
            Inventory RestonInventory = new Inventory("Reston");
            Inventory HerndonInventory = new Inventory("Herndon");
            Inventory SterlingInventory = new Inventory("Sterling");
            ...
            if (tryLoadDatabase)
            {
                ...
                // rebuild topping lists for all pizzas in pizza lit
                foreach (var pizza in pizzaList)
                {
                    pizza.BuildToppingList();
                }
                ...
                {
                    if (inventory.StoreLocation == "Reston") RestonStorefront.StoreInventory = inventory;
                    if (inventory.StoreLocation == "Herndon") HerndonStorefront.StoreInventory = inventory;
                    if (inventory.StoreLocation == "Sterling") SterlingStorefront.StoreInventory = inventory;
                }
                ...
                // try to assign orders to matching stores and customers
                if (orderList.Count != 0)
                {
                    foreach (var order in orderList) // goes through all orders and assigns to store and customer order histories
                    {
                        foreach (var store in storeList) // searches for matching store
                        {
                            if (order.OrderLocation == store.StoreLocation)
                            {
                                store.StorefrontOrderHistory.Add(order);
                            }
                        }
                        foreach (var customer in customerList) // searches for matching customer
                        {
            ...
            if (tryDeserialize)
            {
                Console.WriteLine("Attempting to deserialize...");
                ...
                switch (option)
                {
                    case "Order":
                        ...
                        else // no index found for customer, need to add new customer to the list
                        {
                            ...
                            pizzaRepository.AddUser(currentCustomer);
                        ...
                        //pizzaRepository.UpdateUser(currentCustomer);

                        pizzaRepository.UpdateInventory(currentStorefront.StoreInventory);

                        pizzaRepository.UpdateLocation(currentStorefront);
                    ...
                    case "Search Users":
                        ...
                        {
                            optionList = new List<string> { "Display Latest Order", "Display Order History", "Display Details", "Quit" };
